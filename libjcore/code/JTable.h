...
JTable::GetRowHeight
	...
	const
{
	JRunArrayIterator iter(*itsRowHeights, JListT::kStartBefore, index);
	JCoordinate h;
	iter.Next(&h);
	return h;
}
...
JTable::GetColWidth
	...
	const
{
	JRunArrayIterator iter(*itsColWidths, JListT::kStartBefore, index);
	JCoordinate w = 0;
	iter.Next(&w);
	return w;
}
