...
const JSize kGDefaultFontSize					= 12;
...
GPrefsMgr::GetDefaultMonoFontID()
{
	if (IDValid(kGDefaultMonoFontID))
		...
		dataStream >> name;
		JFontID id;
		ok = JXGetApplication()->GetCurrentDisplay()->GetXFontManager()->GetFontID(name, &id);
		if (ok)
			{
			return id;
			}
		ok = JXGetApplication()->GetCurrentDisplay()->GetXFontManager()->GetFontID("6x13", &id);
		assert(ok);
		SetDefaultMonoFont("6x13");
		return id;
		}
